...
import com.intellij.openapi.vcs.history.VcsUserMappingProvider
import com.intellij.openapi.vcs.history.VcsUserMappings
 ...
 */
@Service
class GitMailmapUserMapper(myProject: Project) : VcsUserMappingProvider, Disposable {
  ...
  private data class Mappings(
    ...
    override fun get(user: VcsUser): VcsUser? {
      // Note: Git documentation does not define the precedence of statements
      myUserToUserMappings[user.lowercase()]?.let { return it }
      myEmailToUserMappings[user.email]?.let { return it }
      myEmailToNameMappings[user.email]?.let { return VcsUserImpl(it, user.email) }
      myEmailToEmailMappings[user.email]?.let { return VcsUserImpl(user.name, it) }
      return null
    }
  }
  ...
  override fun getMappings(vcsRoot: VirtualFile): VcsUserMappings? = myRootsAndMappings[vcsRoot]

  internal fun onRepositoryUpdate(repositories: List<GitRepository>) {
  ...
  class MyRepositoryListener(myProject: Project) : VcsRepositoryMappingListener {
    private val repositoryManager = VcsRepositoryManager.getInstance(myProject)
    private val mailmap = myProject.getService(GitMailmapUserMapper::class.java)

    override fun mappingChanged() {
      mailmap.onRepositoryUpdate(repositoryManager.repositories.filterIsInstance(GitRepository::class.java))
    }
  }
